...
与 [darken](#color-darken) 类似，但是方向相反。
...
与 [saturate](#color-saturate) 类似，但是方向相反。
...
返回颜色名称。如果改颜色没有名称，则返回十六进制 RGB 字符串。
...
与 [rgb](#color-rgb) 类似，但是将 alpha 通道添加到返回的数组中。
...
与 [hsl](#color-hsl) 类似，返回一个包含 hue、saturation 和 value 的数组。
...
类似 [rgb](#color-rgb)，但返回的通道范围是 0 到 1，而不是 0 到 255。
...
将颜色从 CIELab 色彩空间转换为 RGB 时，颜色通道会被裁剪为 0 到 255 的范围。该范围之外的颜色可能在自然界中存在，但无法在 RGB 显示器上显示（例如紫外线）。你可以使用 `color.clipped` 来测试颜色是否被此裁剪。
...
<script setup>
  ...
  const c1 = chroma('red').alpha(0.5).hex()
  const c2 = chroma('hotpink').darken().hex()
  const c3 = chroma('hotpink').darken(2).hex()
  const c4 = chroma('hotpink').darken(2.6).hex()
  const c5 = chroma('hotpink').brighten().hex()
  const c6 = chroma('hotpink').brighten(2).hex()
  const c7 = chroma('hotpink').brighten(3).hex()
  const c8 = chroma('slategray').saturate().hex()
  const c9 = chroma('slategray').saturate(2).hex()
  const c10 = chroma('slategray').saturate(3).hex()
  const c11 = chroma('hotpink').desaturate().hex()
  const c12 = chroma('hotpink').desaturate(2).hex()
  const c13 = chroma('hotpink').desaturate(3).hex()
  const c14 = chroma('skyblue').set('hsl.h', 0).hex()
  const c15 = chroma('hotpink').set('lch.c', 30).hex()
  const c16 = chroma('orangered').set('lab.l', '*0.5').hex()
  const c17 = chroma('darkseagreen').set('lch.c', '*2').hex()
  const c18 = chroma('white').luminance(0.5).hex()
  const c19 = chroma('aquamarine').luminance(0.5).hex()
  const c20 = chroma('hotpink').luminance(0.5).hex()
  const c21 = chroma('darkslateblue').luminance(0.5).hex()
  const c22 =chroma('aquamarine').luminance(0.5).hex()
</script>
